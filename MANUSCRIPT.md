...
[PAGE 1 · pages/nature-of-the-manual-opening.html — built]
[PLATE]
...
[PAGE 4 · pages/nature-of-the-manual-alchemical-book-plate.html — built]
[BODY]
...
[PAGE 5 · pages/nature-of-the-manual-observation.html — built]
[BODY]
...
[PAGE 10 · pages/chapter-2-structure-of-reality-opening.html — built]
[PLATE]
...
[/BODY]
[/PAGE 14]
(this page's own infographic moved OUT — see PAGE 14.5 below — so the page that used to carry it now carries only the prose bumped off PAGE 13, plus the "purpose is threefold" paragraph it always had. Deliberately no plate here any more: at 78w/2p this page is well under the usual ceiling and reads a little sparse on its own, but there wasn't a way to also keep an infographic-sized image on the very next page without one page in the pair carrying less text than usual. Flagged as a known trade-off, not an oversight — happy to pull more content in if a fuller page is wanted here.)

[PAGE 14.5 · pages/chapter-2-deriving-the-optimal-strategy.html — built, NEW]
[BODY]
...
[/BODY]
[/PAGE 14.5]
(a genuinely new page, inserted between the old PAGE 14 and PAGE 15 — not just a reflow. The infographic was asked to go as large as possible (requested progression: 55% -> 65%, which overflowed and clipped against the folio -> explicitly requested up to 83%, "same exact thing" as PAGE 13's plate). At 83%, the paragraph pair that used to lead into it ("The purpose is therefore threefold... present volume... practical deployment...") plus the two paragraphs that used to follow it ("Most manuals..." / "This Manual relieves...") together add up to far more text than can sit next to a plate that size on ONE page, even using only the bare minimum of couching text and pushing everything else forward — the deficit was real, not a formula error, so reflowing sentence-by-sentence couldn't solve it. Splitting the surrounding prose across two pages instead of one is the fix: this page keeps only the minimum lead-in ("present volume/practical deployment", reunited as originally written) and the minimum follow-on ("Most manuals..."), both satisfying the plate-must-have-text-on-both-sides rule with as little text as the rule allows, so the plate itself gets almost the whole page. "This Manual relieves the reader..." — the paragraph that used to follow "Most manuals..." on the same page — moved on to PAGE 15 instead.)
...
[/BODY]
[/PAGE 15]
(picks up "This Manual relieves the reader..." from PAGE 14.5, reuniting it with "Having established that strategy...", which was already here from an earlier reflow — restoring that pair to a single paragraph as originally written. Now 211w/4p: 1 word past the nominal 210-word ceiling, but with only 4 paragraphs (well under the 6-paragraph danger zone that's actually what causes overflow at that word count), this has real margin the same way outcome-asymmetry's exact 210w/6p did — not verified against an actual render. Also has its own added paragraph break before "Once the objective..." from an earlier pass, purely a readability split, no words moved for that one. This is the last page in the chapter's current run — nothing further to cascade into if this page turns out tighter than expected.)

Running-head note: inserting PAGE 14.5 shifts every page after it by one position, which flips which of the two alternating running-heads (“The Reality Manual Vol I: First Principles” / “Chapter II · The Structure of Reality”) belongs on each page from PAGE 14.5 onward — updated PAGE 14.5 itself and PAGE 15 (chapter-2-the-rules-of-reality.html) to match their new parity. Nothing before PAGE 14.5 is affected.
