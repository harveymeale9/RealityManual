...
[/BODY]
[/PAGE 14]
(the "present volume / practical deployment" paragraph is reunited here as it originally stood — the earlier PAGE 13/14 sentence-split that separated it is no longer needed now that PAGE 13 has its own plate to fill the page. "Having established that strategy, the second volume shall concern itself with its practical application throughout the ordinary circumstances of life." — the second sentence of what used to close this page — moved on to PAGE 15 instead; see the note below for why. The infographic itself was first sized to 47% on the word/image-budget formula alone, but a real screenshot showed that formula was too conservative here — a large empty gap remained below the last paragraph before the folio. Sized up from the screenshot's own proportions instead (comparing the visible gap's height against the plate's own rendered height) to 65%, since this is a plate the user specifically called out as important and wants as large as the page allows. Formula-only sizing without a render to check against is what produced the undersized 47% in the first place — prefer an actual screenshot over the formula when the two disagree.)
